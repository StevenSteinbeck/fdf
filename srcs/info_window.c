...
/*   Updated: 2018/06/25 18:54:57 by stestein         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	mlx_string_put(env->mlx, env->win, 10, 0,
			0x00FF0000, "USAGE GUIDE:");
	mlx_string_put(env->mlx, env->win, 10, 30,
			0x00FFFFFF, "Mouse drag: rotate x/y axis");
	mlx_string_put(env->mlx, env->win, 10, 50,
			0x00FFFFFF, "Mouse scroll: zoom");
	mlx_string_put(env->mlx, env->win, 10, 70,
			0x00FFFFFF, "Arrow Keys: move around");
	mlx_string_put(env->mlx, env->win, 10, 90,
			0x00FFFFFF, "Mouse scroll: zoom");
	mlx_string_put(env->mlx, env->win, 10, 110,
			0x00FFFFFF, "Z/X Keys: spin around z axis");
	mlx_string_put(env->mlx, env->win, 10, 130, 0x00FFFFFF, "c: color modes");
	mlx_string_put(env->mlx, env->win, 10, 150,
			0x00FFFFFF, "p: show only points");
	mlx_string_put(env->mlx, env->win, 10, 170,
			0x00FFFFFF, "r: reset everything");
...
void	color_status(t_envars *env)
{
	...
	if (env->color_style == 2)
		mlx_string_put(env->mlx, env->win, 666, 88,
				0x00FFFF00, "YELLOW APPLIED");
